...
def calc_metrics(df: pd.DataFrame, station: str, date: str) -> pd.DataFrame:
    ...
    with warnings.catch_warnings(record=True) as _:
        ...
        for i, window in enumerate(station_rolling):
            ...
            metric_data["datetime_i"].append(df_station.index[i])
            metric_data["value_i"].append(df_station.iloc[i, 0])
    ...
    df_result.index = pd.to_datetime(df_result["datetime_i"])
    df_result.drop(columns=["datetime_i"], inplace=True)
