...
Outputs:
  outputs/lead-lag/global_rank.csv
  outputs/lead-lag/event_summary.csv
  tables/lead-lag/rank_global.tex
  tables/lead-lag/event_summary.tex
  figures/lead-lag/heatmaps/median_lead_heatmap.png
  figures/lead-lag/violins/lag_violin_<date>.png
...
CFG = {
    ...
    # Salidas
    "out_dir": "outputs/lead-lag",
    "tables_dir": "tables/lead-lag",
    "fig_dir": "figures/lead-lag",
}
            ...
            rows.append(
                {
                    ...
                    "station": st.upper(),
                    "onset_idx": t0_idx,
                    "onset_time": t.iloc[t0_idx] if 0 <= t0_idx < n else pd.NaT,
                    "invariant": col,
                    "lag_idx": lag,
                    "lag_time": t.iloc[lag] if 0 <= lag < n else pd.NaT,
                    "lag_star": (t.iloc[lag] - t.iloc[t0_idx]).total_seconds() / 3600.0,
...
def heatmap_median_lead(e, events, outpath):
    ...
    ax.set_title("Median lead by invariant and event (hours; negative = precedes)")
