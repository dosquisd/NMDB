...
FIXED_STATION = (
    "CALG"  # <-- cámbiala por "JBGO", "DOMC", "NAIN", "NEWK", "APTY", "OULU"
)
# "SOPO", etc.
...
def load_station_df(date, station):
    patt1 = (
        DATA_DIR / date / f"{station}_metrics-windowsize_130-ewm_alpha_0.15.csv"
    )
    ...
    if not f.exists():
        return None
    df = pd.read_csv(f)
    tcol = _find_col(df, TIME_CAND)
    ccol = _find_col(df, COUNT_CAND)
...
def marker_columns(df):
    ...
    # filtra NaN-only
    cols = [
        c for c in cols if pd.to_numeric(df[c], errors="coerce").notna().any()
    ]
    return cols
...
def plot_overlay(date, station, markers=PANEL_MARKERS, save_panel=True):
    ...
    ax.set_xlabel("time")
    ax.set_ylabel("standardized derivative")
