...
CFG = {
    ...
    "invariants": [
        "permutation_entropy",
        "sample_entropy", "sampen", "approximate_entropy", "ap_entropy",
        "spectral_entropy", "shannon_entropy",
        "lzc", "lempel_ziv", "lempel_ziv_complexity",
        "hurst_dfa", "dfa_hurst", "hurst",
        "higuchi_fd", "katz_fd", "petrosian_fd",
        "corr_dimension", "correlation_dimension"
    ],
...
def list_station_files(date):
    ...
    files = glob.glob(f"{base}/{date}/*_metrics-windowsize_130-ewm_alpha_0.15.csv")
    ...
    for f in sorted(files):
        ...
        if st not in by_station or f.endswith("ewm_alpha_0.15.csv"):
            by_station[st] = f
...
def read_station_df(filepath):
    ...
    if tcol is None:
        raise ValueError(f"No time column found in {filepath}. Available: {df.columns.tolist()}")
...
def infer_marker_columns(df):
    ...
    extra = [c for c in df.columns if c not in ["__time__","__count__"]
             and c not in CFG["time_cols"] and c not in CFG["count_cols"]
             and df[c].dtype != 'O']  # no strings
...
def process_event(date):
    ...
    for st, fpath in files.items():
        ...
        if valid_count < 10:
            print(f"[WARN] Too few valid count points in {Path(fpath).name} — skipping {st} {date}")
            continue
    
        # si no hay conteo usable, saltar estación
        if df["__count__"].isna().all():
            print(f"[WARN] Missing count column in {Path(fpath).name} — skipping station {st} for {date}")
            continue

        x = df["__count__"].astype(float).to_numpy()
        # suavizado + zscore + derivada
        ...
        n = len(dx)
        mask_pre = np.zeros(n, dtype=bool); mask_pre[:max(t0_idx,0)] = True
        ...
        for col in mcols:
            ...
            B = CFG["B"]; block = CFG["block"]
            bs = moving_block_bootstrap(md, block, B)
            corr_obs = safe_corr_at_lag(md, dx, lag)
            if np.isnan(corr_obs):
                pval = 1.0
            elif corr_obs >= 0:
                pval = float((np.apply_along_axis(lambda a: safe_corr_at_lag(a, dx, lag), 1, bs) >= corr_obs).mean())
            else:
                pval = float((np.apply_along_axis(lambda a: safe_corr_at_lag(a, dx, lag), 1, bs) <= corr_obs).mean())

            # desfase de picos en ventana ±W
            w = min(CFG["W"], n//2)
            i_cnt_min = np.argmin(dx[max(0, t0_idx-w): min(n, t0_idx+w)]) + max(0, t0_idx-w)
            i_mrk_min = np.argmin(md[max(0, t0_idx-w): min(n, t0_idx+w)]) + max(0, t0_idx-w)
...
def summarize_global(df):
    ...
    g = (df.groupby("invariant")
           .agg(median_lag=("lag_star","median"),
           ...
           .reset_index())
    # score Rk
    Lref = 120.0; w1,w2,w3 = 0.4,0.3,0.3
...
def tex_rank_table(g):
    ...
    for _, r in g.iterrows():
        rows.append(f"{r['invariant'].replace('_','\\_')} & {r['median_lag']:.1f} & {r['iqr_lag']:.1f} & {r['sig_pct']:.0f} & {r['Rk']:.2f} \\\\")
    body = "\n".join(rows)
    tex = r"""
\begin{table}[t]
\centering
...
def heatmap_median_lead(e, events, outpath):
    piv = e.pivot(index="invariant", columns="date", values="median_lag").reindex(columns=events)
    if piv.empty: return
    fig, ax = plt.subplots(figsize=(10, max(4, 0.35*len(piv))))
    im = ax.imshow(piv.values, aspect="auto")
    ax.set_yticks(range(len(piv))); ax.set_yticklabels([s.replace("_"," ") for s in piv.index])
    ax.set_xticks(range(len(events))); ax.set_xticklabels(events)
...
def violin_lags(df, date, outpath):
    ...
    ax.set_yticks(np.arange(1, len(labels)+1)); ax.set_yticklabels(labels)
...
def main():
    out_dir = Path(CFG["out_dir"]); out_dir.mkdir(exist_ok=True)
    tab_dir = Path(CFG["tables_dir"]); tab_dir.mkdir(exist_ok=True)
    fig_dir = Path(CFG["fig_dir"]); fig_dir.mkdir(exist_ok=True)
    ...
    # heatmap global
    heatmap_median_lead(e, CFG["events"], fig_dir / "heatmaps" / "median_lead_heatmap.png")
